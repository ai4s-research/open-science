...
並行して作業するのに、2 つ目のサーバーも 2 つ目のワークスペースも必要あり
ません。1 つの `osd server` がすでに複数のセッションを同時に実行しています。
ゲートウェイは接続ごとにスレッドを立て、セッションの経路上にグローバル
ロックはありません — セッション同士の作業を分けているのは、それぞれが
固定されているフォルダーです。`osd session new --project NAME` はワーク
スペース内のプロジェクトフォルダー（`osd project new NAME` で作成）で
セッションを実行し、`osd session new --directory DIR` は任意の別フォルダーに
固定します。

プロジェクト別の MCP もフォルダーに従います。
`<フォルダー>/.opencode/opencode.json` に宣言した MCP サーバーは、その
フォルダーで作業するセッションにだけ適用され、すべてのセッションが同じ
サイドカーを共有します。ここで順序が重要になるのはキャッシュの規則です。
フォルダーの opencode 設定は、サイドカーがそのフォルダーを最初に使うときに
一度だけ読み込まれ、以後キャッシュされます。この設定は、そのフォルダーに
セッションを向ける前に書いてください。`osd session new --project NAME`
（または `--directory DIR`）の時点で、まだ 1 ターンも走っていないのに設定は
読み込まれてキャッシュされます——つまり「最初のターンの前」ではすでに
手遅れです。後から変更してもサーバーの再起動まで反映されず、それまでの間、
セッションは動き、成功を報告し、以前の設定を使います。

このどれにも、ワークベンチの 2 つ目のインストールは必要ありません。

### 分離された状態ディレクトリ(--state-dir)

`osd server --state-dir DIR`（または `OSD_STATE_DIR=DIR`）は、そのインスタンス
が書き出すものすべて — セッションデータベース、サイドカー設定、ワークスペース
記録、ログ — を、プラットフォームのデータディレクトリではなく DIR の下に置き
ます。ほかは何も変わりません。既定のワークスペースは実際の Documents
フォルダーのままで、デスクトップアプリはヘッドレス用の上書きを経由しない
ため影響を受けません。

これは並行処理を増やす方法では**ありません** — 1 つのサーバーがすでに複数の
セッションを同時に実行しています（上記参照）。これは、1 台のマシン上で
本当に独立したインストールのためのものです。デスクトップアプリの隣で動く
...
`full` は既定ではなく、意図した選択です。エージェントはワークスペース内に留まりますが、確認で止まることはなくなります。`osd approval set approve` ですべての規則が戻ります。`osd approval set full` はランタイム自身の設定を書き換えるため、マシン本体で実行する必要があります — `--gateway` 経由ではなく SSH で。ゲートウェイは設定の書き込みを意図的に拒否し、黙って何もしないのではなくその旨を伝えます。
