...
import { connectMacOSProviderSocket } from '../../../../../runtime/computer/macos-native-provider-socket'
...
describe('connectMacOSProviderSocket', () => {
  it('removes socket listeners after a connection error', async () => {
    ...
    try {
      ...
      const promise = connectMacOSProviderSocket('/tmp/osd-computer.sock', 50)
  ...
  it('destroys the pending socket and removes listeners when aborted', async () => {
    ...
    const promise = connectMacOSProviderSocket('/tmp/osd-computer.sock', 5_000, abort.signal)
    await Promise.resolve()
    abort.abort()
