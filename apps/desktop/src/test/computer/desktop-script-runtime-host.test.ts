...
import type { ProcessSpec } from '../../../../../runtime/computer/process'
import type { RuntimeChildProcess } from '../../../../../runtime/computer/desktop-script-serve-channel'
import { DesktopScriptRuntimeHost, isRuntimeHostUnavailable } from '../../../../../runtime/computer/desktop-script-runtime-host'
...
class FakeRuntimeChild extends EventEmitter {
  ...
  pendingId(): number {
    ...
    return requests[requests.length - 1]?.requestId as number
  }
...
function createHost(
  ...
  const host = new DesktopScriptRuntimeHost('C:\\osd\\runtime.ps1', {
    ...options,
...
describe('DesktopScriptRuntimeHost', () => {
  ...
  it('stops respawning a helper that dies on every second operation', async () => {
    ...
    for (let round = 0; round < 3; round++) {
      ...
      await settle()
      children[children.length - 1]?.exit(1, 'boom')
  ...
  it('returns to RemoteSigned when Bypass does not start a helper either', async () => {
    ...
    const recovered = host.request({ tool: 'handshake' })
    await settle()
    expect(specs[specs.length - 1]?.args).not.toContain('Bypass')
    children[children.length - 1]?.respond({ ok: true, capabilities: {} })
  ...
  it('gives up only after repeated start failures, then serves from the host again after the cooldown', async () => {
    ...
    expect(warnings[warnings.length - 1]).toMatch(/recovered/)
    host.dispose()
  })
