...
describe("imageAttachmentParts", () => {
  ...
  it("sends an image as a data-URL part (a file:// url loses the whole turn)", async () => {
    ...
    expect(await imageAttachmentParts(["figure.png"])).toEqual({
      parts: [{ filename: "figure.png", mime: "image/png", url: "data:image/png;base64,UE5H" }],
      dropped: [],
    });
  });
  ...
  it("reads nothing when there are no attachments", async () => {
    ...
    expect(await imageAttachmentParts([])).toEqual({ parts: [], dropped: [] });
  ...
  it("leaves non-images to the agent's own file tools", async () => {
    ...
    // Not sending a CSV is the design, so it is not reported as dropped.
    expect(await imageAttachmentParts(["data.csv"])).toEqual({ parts: [], dropped: [] });
  });
  ...
  it("drops an unreadable attachment without failing the others", async () => {
    ...
    expect(await imageAttachmentParts(["gone.png", "kept.jpg"])).toEqual({
      parts: [{ filename: "kept.jpg", mime: "image/jpeg", url: "data:image/jpeg;base64,SkZH" }],
      // The one that failed is NAMED, so the send can say the picture did not
      // go — the prompt text still mentions the file either way.
      dropped: ["gone.png"],
    });
  });
