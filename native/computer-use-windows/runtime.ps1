# Open Science Desktop Windows computer-use bridge: a small UI Automation
# adapter. One JSON operation in, one JSON response out.
#
...
public static class OsdDesktopWin32 {
...
function Assert-OsdProcessAllowed($Process) {
...
function Test-OsdBrowserProcess($Process) {
...
function Get-OsdRootElement($Process) {
...
function Get-OsdWindowFrame($Process, $RootElement) {
    $rect = New-Object OsdDesktopWin32+RECT
    if ([OsdDesktopWin32]::GetWindowRect([IntPtr]$Process.MainWindowHandle, [ref]$rect)) {
        return New-OsdFrame $rect.Left $rect.Top ($rect.Right - $rect.Left) ($rect.Bottom - $rect.Top)
    }

    try {
        $bounds = $RootElement.Current.BoundingRectangle
        if (-not $bounds.IsEmpty) {
            return New-OsdFrame $bounds.X $bounds.Y $bounds.Width $bounds.Height
        }
    } catch {}
    $null
}

function Get-OsdWindowId($Process) {
    [int64]$Process.MainWindowHandle
}

function Get-OsdAppName($Process) {
...
function New-OsdAppRecord($Process) {
    [pscustomobject]@{
        name = Get-OsdAppName $Process
...
function Assert-OsdWindowTarget($Process, $WindowId, $WindowIndex) {
    if ($null -ne $WindowIndex -and [int]$WindowIndex -ne 0) {
        throw "windowNotFound(`"$WindowIndex`")"
    }
    if ($null -ne $WindowId -and [int64]$WindowId -ne (Get-OsdWindowId $Process)) {
        throw "windowNotFound(`"$WindowId`")"
    }
}

function Restore-OsdWindow($Process) {
    if ($Process.MainWindowHandle -eq 0) { return }
    [void][OsdDesktopWin32]::ShowWindow([IntPtr]$Process.MainWindowHandle, 9)
    [void][OsdDesktopWin32]::SetForegroundWindow([IntPtr]$Process.MainWindowHandle)
}

function Test-OsdWindowFocused([IntPtr]$WindowHandle) {
    [OsdDesktopWin32]::GetForegroundWindow() -eq $WindowHandle
}

function Wait-OsdWindowFocused([IntPtr]$WindowHandle, [int]$TimeoutMilliseconds) {
    $stopwatch = [Diagnostics.Stopwatch]::StartNew()
    while ($stopwatch.ElapsedMilliseconds -lt $TimeoutMilliseconds) {
        if (Test-OsdWindowFocused $WindowHandle) { return $true }
        Start-Sleep -Milliseconds 50
    }
    Test-OsdWindowFocused $WindowHandle
}

function Assert-OsdKeyboardFocus([IntPtr]$WindowHandle, $Operation) {
    if (Test-OsdWindowFocused $WindowHandle) { return }
    if ([bool]$Operation.restoreWindow) {
        if (Wait-OsdWindowFocused $WindowHandle 500) { return }
...
function Get-OsdElementFrame($Element, $WindowFrame) {
    try {
        ...
        New-OsdFrame ($bounds.X - $WindowFrame.x) ($bounds.Y - $WindowFrame.y) $bounds.Width $bounds.Height
    } catch {
        $null
    }
}

function Get-OsdProperty($Element, [string]$Name) {
    try { [string]$Element.Current.$Name } catch { "" }
}

function Get-OsdRuntimeId($Element) {
    try { @($Element.GetRuntimeId()) } catch { @() }
}

function Test-OsdSensitiveElement($Element) {
    try {
        if ($Element.Current.IsPassword) { return $true }
    } catch {}
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $parts = @(
        (Get-OsdProperty $Element "LocalizedControlType"),
        $controlType,
        (Get-OsdProperty $Element "Name"),
        (Get-OsdProperty $Element "AutomationId"),
        (Get-OsdProperty $Element "ClassName")
    )
...
function Get-OsdValueText($Element) {
    try {
        if (Test-OsdSensitiveElement $Element) { return "[redacted]" }
...
function Get-OsdActions($Element) {
...
function Get-OsdMeaningfulActions($Actions) {
    $noisy = @("Invoke", "ScrollToVisible", "ShowMenu")
    @($Actions | Where-Object { $noisy -notcontains $_ })
}

function Format-OsdSnapshotText([string]$Text) {
    if ([string]::IsNullOrWhiteSpace($Text)) { return "" }
    (($Text -replace "\s+", " ").Trim())
}

function Format-OsdValueSegment([string]$RoleKey, [string]$Title, [string]$Value) {
    $clean = Format-OsdSnapshotText $Value
...
function Test-OsdSuppressChildren([string]$RoleKey, [string]$Title, [string]$Value, [string]$Summary) {
    $hasCompactLabel = -not [string]::IsNullOrWhiteSpace($Title) -or -not [string]::IsNullOrWhiteSpace((Format-OsdSnapshotText $Value)) -or -not [string]::IsNullOrWhiteSpace((Format-OsdSnapshotText $Summary))
...
function Get-OsdTextSnippets($Element, [int]$Limit = 6, [int]$MaxDepth = 3) {
    $values = New-Object System.Collections.Generic.List[string]
    $seen = New-Object System.Collections.Generic.HashSet[string]

    function Visit-OsdText($Node, [int]$Depth) {
        ...
        if ($role -match "text|link|label") {
            foreach ($raw in @((Get-OsdProperty $Node "Name"), (Get-OsdValueText $Node))) {
        ...
        try {
            ...
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-OsdText $children.Item($i) ($Depth + 1)
                if ($values.Count -ge $Limit) { return }
            }
        } catch {}
    }

    Visit-OsdText $Element 0
    @($values.ToArray())
}

function Test-OsdPlainTextSubtree($Element, [int]$MaxDepth = 4) {
    $script:sawOsdText = $false
    $allowed = @("pane", "group", "custom", "unknown", "text", "link", "image")

    function Visit-OsdPlainText($Node, [int]$Depth) {
        if ($Depth -gt $MaxDepth) { return $false }
        $role = try { [string]$Node.Current.LocalizedControlType } catch { "" }
        $roleKey = $role.ToLowerInvariant()
        if ($allowed -notcontains $roleKey) { return $false }
        if ($roleKey -match "text|link") { $script:sawOsdText = $true }
        if (@(Get-OsdMeaningfulActions @(Get-OsdActions $Node)).Count -gt 0) { return $false }
        try {
            $children = $Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition)
            for ($i = 0; $i -lt $children.Count; $i++) {
                if (-not (Visit-OsdPlainText $children.Item($i) ($Depth + 1))) { return $false }
            }
        } catch {}
        return $true
    }

    (Visit-OsdPlainText $Element 0) -and $script:sawOsdText
}

function New-OsdElementRecord($Element, [int]$Index, $WindowFrame) {
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $nativeWindowHandle = try { [int64]$Element.Current.NativeWindowHandle } catch { 0 }
    [pscustomobject]@{
        index = $Index
        runtimeId = @(Get-OsdRuntimeId $Element)
        automationId = Get-OsdProperty $Element "AutomationId"
        name = Get-OsdProperty $Element "Name"
        controlType = $controlType
        localizedControlType = Get-OsdProperty $Element "LocalizedControlType"
        className = Get-OsdProperty $Element "ClassName"
        value = Get-OsdValueText $Element
        isSelected = Test-OsdElementSelected $Element
        nativeWindowHandle = $nativeWindowHandle
        frame = Get-OsdElementFrame $Element $WindowFrame
        actions = @(Get-OsdActions $Element)
    }
}

function Test-OsdElementSelected($Element) {
    try {
...
function Render-OsdTree($RootElement, $WindowFrame, [bool]$CompactBrowserTabs = $false) {
    ...
    function Visit-OsdNode($Node, [int]$Depth) {
        ...
        $record = New-OsdElementRecord $Node $records.Count $WindowFrame
        $children = @()
        try {
            $children = @($Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition))
        } catch {}
        $meaningfulActions = @(Get-OsdMeaningfulActions $record.actions)
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value)) {
            $snippets = @(Get-OsdTextSnippets $Node 8 4)
            if ($snippets.Count -ge 2 -and (Test-OsdPlainTextSubtree $Node)) {
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value) -and $meaningfulActions.Count -eq 0 -and $null -eq $genericSummary -and $children.Count -le 1) {
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-OsdNode $children.Item($i) $Depth
            }
            return
        }
        ...
        $line = "$($record.index) $role $(Format-OsdSnapshotText $title)".TrimEnd()
        $line += Format-OsdValueSegment $roleKey $title $record.value
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -and $genericSummary -ne $title) {
            $line += ", Text: " + (Format-OsdSnapshotText $genericSummary)
        } elseif ($roleKey -in @("row", "data item", "list item")) {
            $rowSummary = @((Get-OsdTextSnippets $Node 6 3)) -join " "
            if (-not [string]::IsNullOrWhiteSpace($rowSummary) -and $rowSummary -ne $title) {
                $line += ", Text: " + (Format-OsdSnapshotText $rowSummary)
            }
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -or (Test-OsdSuppressChildren $roleKey $title $record.value $genericSummary)) { return }
        $childLineStart = $lines.Count
        for ($i = 0; $i -lt $children.Count; $i++) {
            Visit-OsdNode $children.Item($i) ($Depth + 1)
        }
        if ($CompactBrowserTabs) {
            Compress-OsdRenderedBrowserTabs $records $lines $childLineStart ($Depth + 1)
        }
    }

    Visit-OsdNode $RootElement 0
    [pscustomobject]@{ elements = @($records.ToArray()); lines = @($lines.ToArray()); truncation = $truncation }
}

function Compress-OsdRenderedBrowserTabs($Records, $Lines, [int]$StartLine, [int]$Depth) {
    $tabLineIndexes = New-Object System.Collections.Generic.List[int]
    for ($lineIndex = $StartLine; $lineIndex -lt $Lines.Count; $lineIndex++) {
        if (Test-OsdDirectRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth) {
    ...
    foreach ($lineIndex in $tabLineIndexes) {
        if (Test-OsdActiveRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth $recordsByIndex) {
    ...
    for ($i = $tabLineIndexes.Count - 1; $i -ge 0; $i--) {
        ...
        $recordIndex = Get-OsdRenderedElementIndex ([string]$Lines[$lineIndex]) $Depth
...
function Test-OsdDirectRenderedBrowserTabLine([string]$Line, [int]$Depth) {
    $indent = "`t" * $Depth
...
function Test-OsdActiveRenderedBrowserTabLine([string]$Line, [int]$Depth, $RecordsByIndex) {
    if ($Line.Contains("(selected")) { return $true }
    $recordIndex = Get-OsdRenderedElementIndex $Line $Depth
    ...
    [bool]$record.isSelected -or (Format-OsdSnapshotText $record.value) -eq "1"
}

function Get-OsdRenderedElementIndex([string]$Line, [int]$Depth) {
    $text = $Line.Substring(("`t" * $Depth).Length)
    if ($text -match "^(\d+)") { return [int]$Matches[1] }
    $null
}

function ConvertTo-OsdPngBytes([System.Drawing.Image]$Image) {
    $stream = $null
    try {
...
function New-OsdScreenshotPayload([byte[]]$Bytes, [int]$Width, [int]$Height, [double]$Scale) {
    [pscustomobject]@{
...
function Resize-OsdBitmap([System.Drawing.Bitmap]$Source, [int]$Width, [int]$Height) {
    $resized = $null
    $graphics = $null
    try {
...
function Get-OsdBoundedScreenshotPayload([System.Drawing.Bitmap]$Bitmap) {
    $originalWidth = [int][Math]::Max(1, $Bitmap.Width)
    $originalHeight = [int][Math]::Max(1, $Bitmap.Height)
    $pngBytes = ConvertTo-OsdPngBytes $Bitmap
    if ($pngBytes.Length -le $MaxScreenshotPngBytes) {
        return New-OsdScreenshotPayload $pngBytes $originalWidth $originalHeight 1.0
    }
    ...
    while ($scale -ge $MinScreenshotScale) {
        ...
        $resized = $null
        try {
            $resized = Resize-OsdBitmap $Bitmap $width $height
            $candidateBytes = ConvertTo-OsdPngBytes $resized
            if ($candidateBytes.Length -le $MaxScreenshotPngBytes) {
                return New-OsdScreenshotPayload $candidateBytes $width $height ($width / [double]$originalWidth)
            }
        } finally {
            if ($null -ne $resized) { $resized.Dispose() }
        }
...
function Get-OsdScreenshot([bool]$IncludeScreenshot, $WindowFrame) {
    ...
    try {
        ...
        Get-OsdBoundedScreenshotPayload $bitmap
    } catch {
        $null
    } finally {
        if ($null -ne $graphics) { $graphics.Dispose() }
...
function New-OsdSnapshot([string]$Query, [bool]$IncludeScreenshot, $WindowId = $null, $WindowIndex = $null, [bool]$RestoreWindow = $false) {
    $process = Find-OsdProcess $Query
    if ($RestoreWindow) { Restore-OsdWindow $process }
    Assert-OsdWindowTarget $process $WindowId $WindowIndex
    $root = Get-OsdRootElement $process
    $windowFrame = Get-OsdWindowFrame $process $root
    $tree = Render-OsdTree $root $windowFrame (Test-OsdBrowserProcess $process)
    $screenshot = Get-OsdScreenshot $IncludeScreenshot $windowFrame

    [pscustomobject]@{
        snapshotId = [guid]::NewGuid().ToString()
        app = New-OsdAppRecord $process
        windowTitle = $process.MainWindowTitle
        windowId = Get-OsdWindowId $process
...
function Get-OsdAppList {
    @(Get-OsdWindowProcesses | ForEach-Object {
        New-OsdAppRecord $_
    })
}

function Get-OsdWindowList([string]$Query) {
    $process = Find-OsdProcess $Query
    $root = Get-OsdRootElement $process
    $windowFrame = Get-OsdWindowFrame $process $root
    $x = $null
    ...
    [pscustomobject]@{
        ...
        windows = @([pscustomobject]@{
            ...
            isOffscreen = $false
            screenIndex = $null
            platform = [pscustomobject]@{ backend = "uia"; nativeWindowHandle = Get-OsdWindowId $process }
        })
    }
}

function Get-OsdHandshake {
    [pscustomobject]@{
        platform = "win32"
        provider = "osd-computer-use-windows"
...
function Test-OsdSameRuntimeId($Left, $Right) {
...
function Find-OsdElement($RootElement, $Record) {
    ...
    try {
        ...
        for ($i = 0; $i -lt $descendants.Count; $i++) {
            ...
            if (Test-OsdSameRuntimeId @($candidate.GetRuntimeId()) @($Record.runtimeId)) {
                return $candidate
            }
        }
    } catch {}
...
function Invoke-OsdPrimaryAction($Element) {
    foreach ($pattern in @(
...
function Invoke-OsdNamedAction($Element, [string]$Action) {
    $wanted = ""
...
function Set-OsdElementValue($Element, [string]$Value) {
    try {
        $pattern = $Element.GetCurrentPattern([Windows.Automation.ValuePattern]::Pattern)
...
function Get-OsdRequiredNumber($Value, [string]$Name) {
...
function Get-OsdPositiveInteger($Value, [string]$Name) {
...
function Get-OsdPositiveNumber($Value, [string]$Name) {
    if ($null -eq $Value) { $Value = 1 }
    $number = Get-OsdRequiredNumber $Value $Name
    if ($number -le 0) { throw "$Name must be a positive number" }
    $number
}

function Get-OsdRequiredString($Value, [string]$Name) {
    if ($null -eq $Value) { throw "$Name is required" }
    $text = [string]$Value
...
function Get-OsdScreenPoint($Operation, $WindowFrame) {
    if ($null -ne $Operation.element) {
        throw "stale element frame; run get-app-state again and use a fresh element index"
    }
    $x = Get-OsdRequiredNumber $Operation.x "x"
    $y = Get-OsdRequiredNumber $Operation.y "y"
    @{
...
function Get-OsdElementScreenPoint($Element) {
...
function Send-OsdMouseClick([IntPtr]$WindowHandle, [int]$ScreenX, [int]$ScreenY, [string]$Button, [int]$Count, [string]$Modifiers) {
    [void][OsdDesktopWin32]::SetForegroundWindow($WindowHandle)
    [void][OsdDesktopWin32]::SetCursorPos($ScreenX, $ScreenY)
    ...
    $modifierKeys = @(Get-OsdClickModifierVirtualKeys $Modifiers)
    $clickCount = Get-OsdPositiveInteger $Count "click_count"
    if ($modifierKeys.Count -eq 0) {
        for ($i = 0; $i -lt $clickCount; $i++) {
            [OsdDesktopWin32]::mouse_event($down, 0, 0, 0, [UIntPtr]::Zero)
            Start-Sleep -Milliseconds 35
            [OsdDesktopWin32]::mouse_event($up, 0, 0, 0, [UIntPtr]::Zero)
        }
        return
    }
    for ($i = 0; $i -lt $clickCount; $i++) {
        [OsdDesktopWin32]::SendModifiedClick(
            [byte[]]$modifierKeys,
            [uint32]$down,
            [uint32]$up
        )
...
function Send-OsdDrag([IntPtr]$WindowHandle, $From, $To) {
    [void][OsdDesktopWin32]::SetForegroundWindow($WindowHandle)
    $startX = [int]$From.x
    $startY = [int]$From.y
    $endX = [int]$To.x
    $endY = [int]$To.y
    [void][OsdDesktopWin32]::SetCursorPos($startX, $startY)
    [OsdDesktopWin32]::mouse_event($MouseEvents.LeftDown, 0, 0, 0, [UIntPtr]::Zero)
    for ($step = 1; $step -le 12; $step++) {
        $x = [int][Math]::Round($startX + (($endX - $startX) * $step / 12))
        $y = [int][Math]::Round($startY + (($endY - $startY) * $step / 12))
        [void][OsdDesktopWin32]::SetCursorPos($x, $y)
        Start-Sleep -Milliseconds 20
    }
    [OsdDesktopWin32]::mouse_event($MouseEvents.LeftUp, 0, 0, 0, [UIntPtr]::Zero)
}

function Send-OsdText([IntPtr]$WindowHandle, [string]$Text) {
    [void][OsdDesktopWin32]::SetForegroundWindow($WindowHandle)
    $hasNonAscii = $false
    ...
    if ($hasNonAscii) {
        foreach ($character in $Text.ToCharArray()) {
            [void][OsdDesktopWin32]::PostMessage($WindowHandle, $WindowsMessages.Char, [IntPtr][int][char]$character, [IntPtr]::Zero)
            Start-Sleep -Milliseconds 8
        }
        return
    }
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-OsdSendKeysText $Text))
}

function Get-OsdVirtualKey([string]$Key) {
...
function Send-OsdKey([IntPtr]$WindowHandle, [string]$Key) {
    [void][OsdDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-OsdSendKeysKey $Key))
}

function Get-OsdModifierVirtualKey([string]$Modifier) {
...
function Get-OsdClickModifierVirtualKeys([string]$Modifiers) {
    ...
    @($parts | ForEach-Object { Get-OsdModifierVirtualKey $_ })
}

function Send-OsdHotkey([IntPtr]$WindowHandle, [string]$KeySpec) {
    ...
    if ($parts.Count -gt 1) {
        foreach ($modifier in $parts[0..($parts.Count - 2)]) {
            $prefix += ConvertTo-OsdSendKeysModifier $modifier
        }
    }
    [void][OsdDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait($prefix + (ConvertTo-OsdSendKeysKey $key))
}

function ConvertTo-OsdSendKeysText([string]$Text) {
...
function ConvertTo-OsdSendKeysKey([string]$Key) {
    switch ($Key.ToLowerInvariant()) {
        ...
        default {
            if ($Key.Length -eq 1) { return (ConvertTo-OsdSendKeysText $Key) }
            throw "Unsupported key: $Key"
        }
    }
}

function ConvertTo-OsdSendKeysModifier([string]$Modifier) {
    switch ($Modifier.ToLowerInvariant()) {
...
function Send-OsdPasteText([IntPtr]$WindowHandle, [string]$Text) {
    $previous = $null
    $hadPrevious = $false
    ...
    try {
        ...
        Send-OsdHotkey $WindowHandle "Ctrl+v"
    } finally {
        if ($hadPrevious) {
...
function Invoke-OsdOperation($Operation) {
    $includeScreenshot = -not [bool]$Operation.noScreenshot
    if ($Operation.tool -eq "handshake") {
        return [pscustomobject]@{ ok = $true; capabilities = Get-OsdHandshake }
    }
    if ($Operation.tool -eq "list_apps") {
        return [pscustomobject]@{ ok = $true; apps = @(Get-OsdAppList) }
    }
    if ($Operation.tool -eq "list_windows") {
        $list = Get-OsdWindowList $Operation.app
        return [pscustomobject]@{ ok = $true; app = $list.app; windows = @($list.windows) }
    }
    if ($Operation.tool -eq "get_app_state") {
        return [pscustomobject]@{ ok = $true; snapshot = New-OsdSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex ([bool]$Operation.restoreWindow) }
    }

    $process = Find-OsdProcess $Operation.app
    if ([bool]$Operation.restoreWindow) { Restore-OsdWindow $process }
    Assert-OsdWindowTarget $process $Operation.windowId $Operation.windowIndex
    $root = Get-OsdRootElement $process
    $windowFrame = if ($null -ne $Operation.windowBounds) { $Operation.windowBounds } else { Get-OsdWindowFrame $process $root }
    $element = Find-OsdElement $root $Operation.element
    $fromElement = Find-OsdElement $root $Operation.fromElement
    $toElement = Find-OsdElement $root $Operation.toElement
    $handle = [IntPtr]$process.MainWindowHandle
    if ($Operation.tool -in @("type_text", "press_key", "hotkey", "paste_text")) {
        Assert-OsdKeyboardFocus $handle $Operation
    }
    $action = $null
    ...
    switch ($Operation.tool) {
        "click" {
            ...
            Restore-OsdWindow $process
            $handledByPattern = $false
            $clickCount = Get-OsdPositiveInteger $Operation.click_count "click_count"
            ...
            if (-not $hasModifiers -and $null -ne $element -and $Operation.mouse_button -ne "right" -and $Operation.mouse_button -ne "middle" -and $clickCount -le 1) {
                $handledByPattern = Invoke-OsdPrimaryAction $element
            }
            if (-not $handledByPattern) {
                $point = Get-OsdElementScreenPoint $element
                if ($null -eq $point) { $point = Get-OsdScreenPoint $Operation $windowFrame }
                Send-OsdMouseClick $handle $point.x $point.y $Operation.mouse_button $clickCount $Operation.modifiers
        ...
        "perform_secondary_action" {
            ...
            if (-not (Invoke-OsdNamedAction $element $Operation.action)) {
        ...
        "scroll" {
            $delta = 120 * [int][Math]::Ceiling((Get-OsdPositiveNumber $Operation.pages "pages"))
            ...
            $point = Get-OsdElementScreenPoint $element
            if ($null -eq $point) { $point = Get-OsdScreenPoint $Operation $windowFrame }
            [void][OsdDesktopWin32]::SetForegroundWindow($handle)
            [void][OsdDesktopWin32]::SetCursorPos([int]$point.x, [int]$point.y)
            [OsdDesktopWin32]::mouse_event($mouseEvent, 0, 0, $delta, [UIntPtr]::Zero)
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "scroll"; fallbackReason = $null }
        }
        "drag" {
            $from = Get-OsdElementScreenPoint $fromElement
            ...
            if ($null -eq $from) {
                $from = @{
                    x = $windowFrame.x + (Get-OsdRequiredNumber $Operation.from_x "from_x")
                    y = $windowFrame.y + (Get-OsdRequiredNumber $Operation.from_y "from_y")
                }
            }
            $to = Get-OsdElementScreenPoint $toElement
            ...
            if ($null -eq $to) {
                $to = @{
                    x = $windowFrame.x + (Get-OsdRequiredNumber $Operation.to_x "to_x")
                    y = $windowFrame.y + (Get-OsdRequiredNumber $Operation.to_y "to_y")
                }
            }
            Send-OsdDrag $handle $from $to
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "drag"; fallbackReason = $null }
        }
        "type_text" {
            Send-OsdText $handle (Get-OsdRequiredString $Operation.text "text")
        ...
        "press_key" {
            Send-OsdKey $handle (Get-OsdRequiredString $Operation.key "key")
        ...
        "hotkey" {
            Send-OsdHotkey $handle (Get-OsdRequiredString $Operation.key "key")
        ...
        "paste_text" {
            Send-OsdPasteText $handle (Get-OsdRequiredString $Operation.text "text")
        ...
        "set_value" {
            if ($null -eq $element -or -not (Set-OsdElementValue $element ([string]$Operation.value))) {
    ...
    try {
        $snapshot = New-OsdSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex
    } catch {
        ...
        $snapshot = New-OsdSnapshot $Operation.app $includeScreenshot $null $null
    }
    [pscustomobject]@{ ok = $true; action = $action; snapshot = $snapshot }
}

function Invoke-OsdServeLoop {
    ...
    while ($true) {
        ...
        try {
            ...
            $response = Invoke-OsdOperation $operation
        } catch {
...
if ($Serve) {
    ...
    Write-OsdJson ([pscustomobject]@{ ok = $false; error = "runtime.ps1 requires an operation path or -Serve" })
} else {
    try {
        $operation = Read-OsdOperation $OperationPath
        Write-OsdJson (Invoke-OsdOperation $operation)
    } catch {
        Write-OsdJson ([pscustomobject]@{ ok = $false; error = [string]$_.Exception.Message })
    }
}
