...
public sealed class OsdRenderTestCounter {
    public int FindAll;
}

public sealed class OsdRenderTestBounds {
...
public sealed class OsdRenderTestControlType {
    public string ProgrammaticName = "";
}

public sealed class OsdRenderTestCurrent {
    public string AutomationId = "";
    public OsdRenderTestBounds BoundingRectangle = new OsdRenderTestBounds();
    public string ClassName = "fake";
    public OsdRenderTestControlType ControlType = new OsdRenderTestControlType();
...
public sealed class OsdRenderTestPatternCurrent {
    public bool IsSelected;
    public string Value = "";
}

public sealed class OsdRenderTestPattern {
    public OsdRenderTestPatternCurrent Current = new OsdRenderTestPatternCurrent();
}

public sealed class OsdRenderTestCollection : IEnumerable {
    private readonly OsdRenderTestElement[] values;

    public OsdRenderTestCollection(OsdRenderTestElement[] values) {
        this.values = values;
    }
    ...
    public OsdRenderTestElement Item(int index) {
        return values[index];
    }
...
public sealed class OsdRenderTestElement {
    public readonly List<OsdRenderTestElement> Children =
        new List<OsdRenderTestElement>();
    public OsdRenderTestCounter Counter = new OsdRenderTestCounter();
    public readonly OsdRenderTestCurrent Current = new OsdRenderTestCurrent();
    public bool FailFindAll;
    public int RuntimeIdValue;
    public string ValueText = "";

    public OsdRenderTestCollection FindAll(object scope, object condition) {
        Counter.FindAll++;
        if (FailFindAll) {
            throw new System.InvalidOperationException("defunct node");
        }
        return new OsdRenderTestCollection(Children.ToArray());
    }

    public OsdRenderTestPattern GetCurrentPattern(object pattern) {
        OsdRenderTestPattern result = new OsdRenderTestPattern();
...
function New-TestCounter {
    New-Object -TypeName OsdRenderTestCounter
}
...
function New-TestElement {
    ...
    $element = New-Object -TypeName OsdRenderTestElement
...
$operationPath = Join-Path ([IO.Path]::GetTempPath()) ("osd-runtime-render-test-" + [guid]::NewGuid() + ".json")
try {
