...
import Foundation
import ImageIO
import OsdComputerUseMacOSCore
import ScreenCaptureKit

private let providerName = "osd-computer-use-macos"
...
struct AppDescriptor {
    ...
    var needsManualAccessibilityMode: Bool {
        ...
        return bundleId.hasPrefix("com.google.chrome") ||
            ...
            bundleId == "notion.id" ||
            // Doubao ships its UI in an embedded Chromium ("Doubao Browser
            // Framework.framework"), and without this it exposes a window, a
            // container and the three traffic-light buttons — nothing to act on.
            bundleId.hasPrefix("com.bot.pc.doubao")
    }
...
struct Snapshot {
    ...
    /// This snapshot's own read is the one that switched Chromium's accessibility
    /// tree on, so the tree may not exist yet.
    let enabledManualAccessibility: Bool
    ...
    func withoutScreenshotPayload() -> Snapshot {
        Snapshot(
            ...
            enabledManualAccessibility: enabledManualAccessibility
        )
    }
}
    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrustedSettled() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for Open Science Computer Use. Open Settings > Computer Use in Open Science Desktop, grant Accessibility to Open Science Computer Use, then retry."
            )
        }
        let appElement = AXUIElementCreateApplication(app.pid)
        let justEnabledManualAccessibility = enableManualAccessibilityIfNeeded(appElement, app: app)
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required, and macOS grants it to Open Science itself rather than to this helper. Enable Open Science under Screen Recording in System Settings, or ask for no screenshot to inspect accessibility state only.")
        ...
        return Snapshot(
            ...
            enabledManualAccessibility: justEnabledManualAccessibility
        )
    }
    ...
    private func renderSnapshot(_ snapshot: Snapshot) -> [String: Any] {
        ...
        return [
            "snapshot": [
                ...
                "manualAccessibilityJustEnabled": snapshot.enabledManualAccessibility,
...
/// Turn on Chromium's accessibility tree for apps that ship it switched off.
/// Returns true when THIS call is the one that flipped it, because the tree does
/// not exist yet at that moment — see `buildSnapshot`.
@discardableResult
private func enableManualAccessibilityIfNeeded(_ appElement: AXUIElement, app: AppDescriptor) -> Bool {
    guard app.needsManualAccessibilityMode else {
        return false
    }
    let alreadyOn = boolAttribute(appElement, "AXManualAccessibility") == true
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    if visibleWindowCount > 0 {
        ...
        throw ProviderError.coded("permission_denied", "app '\(app.name)' has visible windows but no accessibility window (AX reads stayed blocked for \(outcome.waitedMs)ms after retries). macOS Accessibility may need Open Science Computer Use toggled off and on again in System Settings.")
    }
...
private func currentSyntheticClickRecipient(
    ...
    case .dismissed:
        // Upstream (Orca) stops here, and that makes coordinate clicking
        // impossible on exactly the apps it exists for. An app that draws its own
        // UI — Chromium, Electron, a custom renderer — can be the focused
        // application and still expose no AX focused window, which arrives here
        // as "nothing is focused" even though the target window is frontmost and
        // sitting under the cursor. Measured on Doubao: five clicks in a row
        // refused with "current: no focused window" while the window was visibly
        // in front.
        //
        // Ask two questions instead, neither of which goes through the tree the
        // app declined to publish: is the target the focused application, and is
        // its window the frontmost one under this point? Together those are a
        // stronger guarantee for a coordinate click than focus alone — focus says
        // nothing about what the cursor is over. The AX hit test below is not
        // usable here for the same reason the tree is not: an app that exposes no
        // elements has none at that position either (measured — it answers
        // noValue, which reads as "dismissed" and refuses all over again).
        guard frontmostApplicationPID() == snapshot.app.pid,
              let frontWindow = frontmostWindowId(at: point)
        else {
            return .dismissed
        }
        return .focused(syntheticClickRecipient(pid: snapshot.app.pid, windowId: frontWindow))
    case .unavailable:
        return .unavailable
    }
}

/// The window id of the frontmost on-screen window covering `point`, in global
/// screen coordinates — or nil when a window of another app is on top there.
///
/// `CGWindowListCopyWindowInfo(.optionOnScreenOnly)` returns windows front to
/// back, so the first hit is the one that would receive the click. Deliberately
/// NOT filtered to the target's own windows: another app's panel sitting over the
/// point is exactly what this has to catch.
private func frontmostWindowId(at point: CGPoint) -> CGWindowID? {
    ...
    for info in infos {
        ...
        // Fully transparent overlays and windows excluded from capture do not take
        // the click, so they must not hide the window that does.
...
/// The process that owns the active application.
///
/// NSWorkspace and not `kAXFocusedApplicationAttribute`: the AX attribute answers
/// "none" for an app that publishes no accessibility tree, which is precisely the
/// case this is asked in. Measured — Doubao frontmost, System Events and
/// NSWorkspace both saying so, and the AX attribute empty.
...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        if ProcessInfo.processInfo.environment["OPENSCIENCE_COMPUTER_USE_SCK_SCREENSHOTS"] == "1",
    ...
    static func click(
        ...
        do {
            ...
            case let .recipientChanged(expected, actual, deliveredPresses):
                // Without the two probes spelled out, "no focused window" covers
                // both "nothing is focused" and "could not tell", and the two have
                // different fixes.
                let actualDescription = actual.map {
                    "pid \($0.ownerPID) window \($0.windowID)"
                } ?? "no focused window; frontmost app pid \(frontmostApplicationPID().map(String.init) ?? "none"), frontmost window at that point \(frontmostWindowId(at: point).map(String.init) ?? "none")"
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable Open Science Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag Open Science Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag Open Science Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let titleText = checking
            ...
            : (ready ? "Computer Use is Ready" : "Enable Open Science Computer Use")
        let title = label(titleText, size: 22, weight: .bold)
        let subtitle = label(
            checking
                ? "Checking Accessibility and Screenshots."
                : (ready
                    ? "Open Science Desktop can use local apps when you ask."
                    : "Grant permissions so Open Science Desktop can use apps when you ask."),
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "osd-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag Open Science Computer Use"
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "osd-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "Open Science Computer Use")
...
/// The one process this helper serves, named by the caller at launch as
/// `--client-pid`.
///
/// Why it is passed in rather than read from `getppid()`: this helper has to be
/// started through LaunchServices (`open -n`) so that macOS judges the TCC
/// grants against THIS bundle's identity instead of whatever launched it —
/// measured, a directly spawned helper is denied Accessibility even when the
/// bundle holds the grant. LaunchServices means the parent is launchd, so the
/// caller states its own pid instead.
// `nonisolated(unsafe)` rather than an actor or a lock: this is written once,
// during argument parsing, before the socket that reads it exists.
nonisolated(unsafe) private var authorizedClientProcessId: pid_t = 0

/// Three things guard the socket: it lives 0600 inside a 0700 directory, every
/// request carries a per-launch token read from a 0600 file, and this. Upstream
/// (Orca) named its own bundle ids here and looked for an Electron sidecar
/// script in the peer's command line; neither exists for this host.
private func isAuthorizedAgentPeer(_ pid: pid_t) -> Bool {
    return authorizedClientProcessId > 0 && pid == authorizedClientProcessId
}
...
private func runAgent(socketPath: String, token: String?) {
    ...
    setenv("OPENSCIENCE_COMPUTER_USE_SCK_SCREENSHOTS", "1", 1)
    app.run()
}
...
private func runStdio() {
    fputs("Open Science Computer Use provider must be launched by Open Science Desktop in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    guard arguments.count >= 2 else {
        fputs("usage: osd-computer-use-macos --agent <socket-path> --token-file <token-path> --client-pid <pid>\n", stderr)
        exit(2)
    }
    ...
    let token = value(after: "--token-file").flatMap { tokenPath in
        try? String(contentsOfFile: tokenPath, encoding: .utf8)
            .trimmingCharacters(in: .whitespacesAndNewlines)
    }
    guard let token, !token.isEmpty else {
        fputs("osd-computer-use-macos --agent requires a non-empty --token-file\n", stderr)
        exit(2)
    }
    guard let clientPid = value(after: "--client-pid").flatMap({ pid_t($0) }), clientPid > 0 else {
        fputs("osd-computer-use-macos --agent requires --client-pid\n", stderr)
        exit(2)
    }
    authorizedClientProcessId = clientPid
    ...
    guard arguments.count >= 2 else {
        fputs("usage: osd-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
