...
按项目配置的 MCP 同样跟着目录走：声明在 `<目录>/.opencode/opencode.json` 里的 MCP 服务器只对在该目录里工作的会话生效，而所有会话共享同一个 sidecar。这里有一条缓存规则决定了先后顺序：一个目录的 opencode 配置只在 sidecar 第一次使用该目录时读取一次，之后一直缓存。所以会话的 MCP 配置必须在该目录的**第一轮开始前**写好——之后再改，要等服务器重启才会生效；在那之前，会话照常运行、报告成功，用的却是旧配置。
